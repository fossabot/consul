...
func TestParseFlags(t *testing.T) {
	tests := []struct {
		...
		{
			args:  []string{`-bind`, `a`},
			flags: Flags{File{BindAddr: pString("a")}, nil},
		},
		{
			args:  []string{`-bootstrap`},
			flags: Flags{File{Bootstrap: pBool(true)}, nil},
		},
		{
		...
		},
		{
			...
			flags: Flags{File{Bootstrap: pBool(false)}, nil},
		},
		{
			args:  []string{`-bootstrap`, `true`},
			flags: Flags{File{Bootstrap: pBool(true)}, nil},
		...
		},
		{
			...
			flags: Flags{File{Datacenter: pString("a")}, nil},
		},
		{
			args:  []string{`-dns-port`, `1`},
			flags: Flags{File{Ports: FilePorts{DNS: pInt(1)}}, nil},
		},
		{
			args:  []string{`-join`, `a`, `-join`, `b`},
			flags: Flags{File{JoinAddrsLAN: []string{"a", "b"}}, nil},
		},
		{
			args:  []string{`-node-meta`, `a:b`, `-node-meta`, `c:d`},
			flags: Flags{File{NodeMeta: map[string]string{"a": "b", "c": "d"}}, nil},
		},
	}
